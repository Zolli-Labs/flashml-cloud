...
|---|---|---|---|
| **G-1** | Live URL that **opens without a login** | P0 | not started |
...
| **D-4** | End-to-end observability; trace ID across the chain **including hibernation**; staged checkpoints; structured logs | One correlation id spanning `session_id` → `sandbox_id` → `job_id` → `task_id` → `lease_id`, persisted append-only, covering the hibernated window | — | P1 | not started |
...
| **D-9** | Cost & efficiency; hibernation saving **quantified** | Measured durations against published rates; the honest baseline argument (dossier §3.1); the OSS-shrinks-hibernation result (§7.3) | FC/OSS | P1 | partial (model done) |
...
| **C-6.2** | Strong isolation (compute + network + storage) | A probe job that **attempts and fails** to read outside its workspace, reach a forbidden host, and see host processes. Paired with the reason we need it: **we execute user-supplied ML code from a submitted repo** | P1 | not started |
...
| **C-6.4** | Stateful sessions | Marker nonce hash, background process identity, **and a warm artifact/dependency cache** all intact across the hibernation boundary | P0 | not started |
| **C-6.5** | ⭐⭐ Hibernation & wake | **Both modes, for two genuinely different waits** — deep hibernation across the long wait for training, light hibernation across short gaps between evaluation shards. Measured wake latency for each; cost quantified against the right baseline | P0 | not started |
| **C-6.6** | Observability (SLS + trace + metrics + alerting + a real debugging story) | Deferred with the SLS decision (§9). P1 substitute: our own append-only ledger plus FC's supported CPU/memory metrics, with the gap stated honestly | P2 | deferred |
