...
2. Verify the sender domain `mail.zolliai.com` — add Resend's DKIM/SPF records to the `zolliai.com` zone. This is a **subdomain**, so it does not move the website. **Check first that the other product's mail on that zone is undisturbed.**
3. **Blocking prerequisite: resolve `EMAIL_REPLY_TO` to a mailbox that actually receives mail — before `RESEND_API_KEY` is set anywhere.** DKIM/SPF verification (step 2) proves `mail.zolliai.com` can *send*; it says nothing about *receiving*, and a sending subdomain has no MX record by default. `render.yaml` ships `hello@mail.zolliai.com` as a placeholder value, not a working address. The declined email's only call to action is "reply to this message" (re-applying is refused by design — `POST /access-request` 409s once decided), so an unresolved reply-to strands exactly the person who most needs a way back in. Do not skip ahead to step 4 until this is a real, monitored mailbox.
