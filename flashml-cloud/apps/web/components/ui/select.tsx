...
// modal={false} by default: Base UI's modal select locks document scroll
// while open, and the console rail is `sticky` — the lock snaps it back to
// its static position, leaving a white gap where the rail was on any
// scrolled page (reported live on the market board's VRAM filter). A
// select that filters a table is not a modal moment; nothing here needs
// the page frozen. Callers can still pass modal={true} for one that does.
