...
const buttonVariants = cva(
  // `rounded-md` rejoins the token system (was a hardcoded `rounded-[7px]`
  // from before `--radius` moved to 6px — see app/globals.css). The size
  // variants' `rounded-[min(var(--radius-md),10px)/12px)]` caps below are
  // now inert (radius-md is always under 10/12px at the new scale) and are
  // deliberately left as written rather than cleaned up here — see the
  // measured-equivalence comment on `default` below, which this same
  // "minimal diff, no unrelated migration" reasoning extends to.
