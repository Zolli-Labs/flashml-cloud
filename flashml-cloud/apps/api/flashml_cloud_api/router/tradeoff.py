...
not help: the buyer's own hardware at zero cost, with no previous point to
compare against (`baseline`), a single task that no fleet parallelizes
(`no_parallelism`), a fleet larger than the task count has any use for
(`beyond_task_count`), a fleet size that costs more than the one before it
but does not finish any sooner because `ceil(task_count / slots)` just
landed on the same step (`no_marginal_gain`), or a fleet size that is
genuinely faster than the one before it (`helps`). That last label is the
entire point of the module: a curve that always slopes downward would be a
sales tool, not a planner, and would spend somebody's money on a machine
...
__all__ = ["ADVICE_BASELINE", "ADVICE_BEYOND_TASK_COUNT", "ADVICE_HELPS",
           "ADVICE_NO_MARGINAL_GAIN", "ADVICE_NO_PARALLELISM",
           "TradeoffPoint", "tradeoff_curve"]

#: The buyer's own hardware, at zero cost. Checked first, ahead of every
#: other advice code: this is the point every other row is compared
#: against, and it has no "point before it" for `no_parallelism`,
#: `beyond_task_count`, `no_marginal_gain` or `helps` to mean anything
#: against. Reusing `helps` here would badge a $0 row "this helps" as if
#: renting had helped, when nothing was rented.
ADVICE_BASELINE = "baseline"
#: More slots than tasks. Spend buys nothing from here on.
ADVICE_BEYOND_TASK_COUNT = "beyond_task_count"
#: One task. No fleet is faster, at any price. Checked first among the
#: non-baseline codes, so it takes priority over every advice code except
#: `baseline`: a single-task job's rented points are never
...
#: This point is genuinely faster than the one before it in the sweep. Never
#: the zero-rented-slots point -- that one is always `baseline`, since it
#: has no "before it" to be faster than.
ADVICE_HELPS = "helps"
...
def tradeoff_curve(
    *,
    ...
    for rented in range(0, max(0, rentable_slots) + 1):
        ...
        if rented == 0:
            # The buyer's own hardware, at no cost. Nothing was rented, so
            # this cannot be a purchase "win" -- it is the baseline every
            # other row is measured against.
            advice = ADVICE_BASELINE
        elif task_count <= 1:
